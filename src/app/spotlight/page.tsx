...
export default function SpotlightPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 pb-16 pt-6 sm:px-6 sm:py-10">
      ...
      {/* Tab Navigation */}
      <div className="scroll-fade flex gap-1 overflow-x-auto rounded-2xl border border-border bg-surface p-1 pr-7 md:p-1.5">
        {tabs.map(tab => (
          <button
            key={tab.key}
            onClick={() => setActiveTab(tab.key)}
            className={`flex-none sm:flex-1 min-w-[132px] sm:min-w-[120px] px-2 md:px-4 py-2 md:py-2.5 rounded-lg text-xs md:text-sm transition-all focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-surface ${
              activeTab === tab.key
      ...
      {activeTab === 'rising' && (
        <div className="space-y-3 animate-fade-in">
          {risingArtists.map((ra, i) => (
            <div
              key={ra.artist.id}
              className="rounded-2xl border border-border bg-surface p-3 transition-all hover:border-border-light md:p-5"
            >
            ...
            <table className="w-full text-xs md:text-sm min-w-[600px]">
              ...
              <tbody>
                {hotLots.map((lot, i) => (
                  <tr
                    key={lot.id}
                    className="border-b border-border/40 hover:bg-surface-hover focus-within:bg-surface-hover transition-colors"
                  >
                    <td className="p-3 text-muted font-mono">{i + 1}</td>
                    <td className="p-3 font-medium text-foreground">{lot.artist.name}</td>
                    <td className="p-3 text-text-secondary italic max-w-[200px]">
                      <a
                        href={lot.lotUrl}
                        target="_blank"
